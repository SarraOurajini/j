...
$article=json_decode($fichierjson ,true);
...
function getArticle($nbreArticles=LENGTH){
     ...
     //affichage des derniers articles 
        ...
        function remove() {
         echo($articleRecupere);
         
        }
        if(isset($_GET['effacer'])) {
          return remove();
        }
       foreach($articleRecupere as $key => $content) {
         echo ('<a href="index.php"><button>Effacer</button></a>');
         $idToRemove= ($content['id']);
